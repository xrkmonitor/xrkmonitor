	...
	uint32_t dwNodeSize, uint32_t dwNodeCount, int32_t *piSharedKey, FunCompare cmp, FunWarning warn)
{
	...
	if(0 == dwNodeSize || 0 == dwNodeCount || dwNodeCount < STATIC_HASH_ROW_MAX || 0 == *piSharedKey || NULL == cmp)
		return -1;

	phash->dwNodeSize = dwNodeSize;
	phash->dwNodeCount = dwNodeCount;
	phash->dwRealNodeCount = 0;
	phash->bAccessCheck = 0;
	...
	for(i=0; i < MT_ATTACH_SHM_TRY_MAX; i++) {
		iRet = MtReport_GetShm_comm((void**)(&phash->pHash),
	...
	uint32_t dwNodeSize, uint32_t dwNodeCount, uint32_t dwSharedKey, FunCompare cmp, FunWarning warn)
{
	...
	if(0 == dwNodeSize || 0 == dwNodeCount || dwNodeCount < STATIC_HASH_ROW_MAX || 0 == dwSharedKey || NULL == cmp)
		return -1;
	...
	char sCheckStr[32] = {0};
	sprintf(sCheckStr, "%u", phash->dwShareMemBytes);
	int iRet = MtReport_GetShm_comm((void**)(&(phash->pHash)),
		dwSharedKey, phash->dwShareMemBytes, 0666|IPC_CREAT, sCheckStr);
	if(iRet == 1) {
		...
		SetApiErrorMsg("MtReport_GetShm_comm failed ret:%d size:%u", iRet, phash->dwShareMemBytes);
		return -2;
	}
