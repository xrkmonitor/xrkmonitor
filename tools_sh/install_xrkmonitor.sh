...
echo "(1/$STEP_TOTAL) 下载并解压库文件: xrkmonitor_lib.tar.gz"
if [ ! -f xrkmonitor_lib.tar.gz ]; then
	wget -q ${XRKMONITOR_HTTP}/xrkmonitor_lib.tar.gz 
else
...
echo "(2/$STEP_TOTAL) 下载运行测试文件: slog_tool"
if [ ! -f slog_tool ]; then
	wget -q ${XRKMONITOR_HTTP}/slog_tool
	chmod +x slog_tool
